...
2. 逐本读题材（短篇读 `拆文库/{书}/_meta.json` 的 `genre_detected`；长篇优先读 `概要.md`，不足时定点读 `全局分析/六维拆书.md` 的题材与规模维度），与本书题材/方向比对，标 同题材 / 弱相关 / 不相关。
...
4. 主产物齐全时，先读 `爆款机制.md` 的读者需求、因果链、心理机制、迁移条件与误用风险，再读 `六维拆书.md`“三维节奏”章节的结构块三轴曲线、峰谷和转折；`chapter_index.csv` 只把块内章号映射到 locator。
5. 写作时按需读取 `六维拆书.md` 的人物关系图谱、双时间线与信息差、三维节奏章节及 `证据与边界.md`，先从 `structure_blocks.csv` 选范围，再用索引回到主对标原文取证。
6. 工作区存在 `灵感库/灵感索引.csv` 时，按本阶段标签只读 Top 3–8 张 `跨书灵感聚合`；不读取 IA/NM。灵感库缺失或零命中不阻塞开书。
...
> **细纲钩子来源**：优先沿本批剧情单元卡已经固化的爆款机制与三维节奏参照设计钩子链；主对标需要具体落点时先查 `structure_blocks.csv` 的缺口/兑现/剩余钩子，再用五列 `chapter_index.csv` 定位原文核证。索引没有 `opening_hook` / `ending_hook` 等语义列。
