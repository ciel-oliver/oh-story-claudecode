...
>
> **公共灵感库**：跨书机制不再靠正文阶段遍历所有副对标。先从工作区 `灵感库/灵感索引.csv` 按当前任务标签只筛 `跨书灵感聚合`，读取 Top 3–8 张 CBA；写作模块不读取单小说合并或原子灵感。具体预算见 [references/cross-book-recall.md](references/cross-book-recall.md)。
>
...
6. **公共灵感召回**只提供抽象机制：写作任务生成 `题材/读者需求/情绪/关系动作/剧情功能/节奏位置/适用阶段/风险` 标签，按索引选 3–8 张 CBA；IA/NM 不进入写作 prompt，且灵感卡不作为风格样本。
7. **自定义文风 `设定/文风.md`** 含实质内容时作为本书权威风格基；否则先从 `structure_blocks.csv` 选匹配块，再用五列 `chapter_index.csv` 定位主对标原文，临时归纳句长、标点、潜台词与段落节奏。自定义文风优先级不因灵感库改变，硬安全线始终优先。
